...
impl NodeRunner {
    // Sends outgoing messages to connected nodes.
    async fn send_outgoing_msgs(&mut self) {
        let messages = self.node.lock().unwrap().durability.omni_paxos.outgoing_messages();
        for msg in messages {
            let receiver = msg.get_receiver();
            if self.node.lock().unwrap().connected_nodes[receiver as usize - 1] {
                let channel = self
                    .outgoing
                    .get_mut(&receiver)
        ...
        loop {
            tokio::select! {
                ...
                _ = datastore_update_interval.tick() => {
                    let current_leader_pid =
                        self.node.lock().unwrap().durability.omni_paxos.get_current_leader();
...
impl Node {
    ...
    // Updates the leader and applies necessary transactions.
    ...
    // Applies replicated transactions to the datastore.
    ...
    // Rolls back unreplicated transactions in the datastore.
    ...
    // Begins a new transaction with the specified durability level.
    pub fn begin_tx(
        &self,
    ...
    // Releases the specified transaction.
    pub fn release_tx(&self, tx: <ExampleDatastore as Datastore<String, String>>::Tx) {
        self.datastore.release_tx(tx);
    }

    // Begins a mutable transaction, only allowed for the leader.
    pub fn begin_mut_tx(
        &self,
    ...
    // Commits a mutable transaction, only allowed for the leader.
    ...
    // Advances the replicated durability offset in the datastore.
...
mod tests {
    ...
    use self::durability::omnipaxos_durability::TransactionLog;
    
    const SERVERS: [NodeId; 7] = [1, 2, 3, 4, 5, 6, 7];
    
    // Helper function to initialize communication channels between nodes.
    ...
    // Helper function to spawn nodes with Tokio runtime.
    fn spawn_nodes(
        runtime: &mut Runtime,
        node_number: u64,
        ...
        if node_number == 0 {
            for pid in SERVERS {
                ...
                let durability =
                    OmniPaxosDurability::new(server_config.clone(), cluster_config.clone()).unwrap();
                let node: Arc<Mutex<Node>> =
                    Arc::new(Mutex::new(Node::new(pid, durability, SERVERS.len() as u64)));
            ...
            for pid in 1..=node_number {
                ...
                let durability =
                    OmniPaxosDurability::new(server_config.clone(), cluster_config.clone()).unwrap();
                let node: Arc<Mutex<Node>> =
                    Arc::new(Mutex::new(Node::new(pid, durability, node_number as u64)));
                let mut node_runner = NodeRunner {
                    node: node.clone(),
                    incoming: receiver_channels.remove(&pid).unwrap(),
                    outgoing: sender_channels.clone(),
    ...
    #[test]
    // Test Case #1: Find the leader and commit a transaction. Show that the transaction is really *chosen*
    // (according to our definition in Paxos) among the nodes.
    fn test_case_1() {
        let mut runtime = create_runtime();
        let nodes = spawn_nodes(&mut runtime, 0);
        std::thread::sleep(WAIT_LEADER_TIMEOUT);
        ...
        println!("Leader Offset: {:?}", leader_offset);
        println!(
            "Follower {} Offset: {:?}",
            pid,
            server.lock().unwrap().durability.get_durable_tx_offset()
        );
        ...
        // Assert that a node which is not a leader cannot commit
    }
    
    #[test]
    // Test Case #2: Find the leader and commit a transaction. Kill the leader and show that another node will be elected
    ...
    #[test]
    // Test Case #3: Find the leader and commit a transaction. Disconnect the leader from the other nodes and continue to commit
    // transactions before the OmniPaxos election timeout. Verify that the transaction was first committed in memory but later rolled back.
    fn test_case_3() {
        let mut runtime = create_runtime();
        let nodes = spawn_nodes(&mut runtime, 0);
        std::thread::sleep(WAIT_LEADER_TIMEOUT);
        ...
        println!(
            "Replicated logs: {:?}",
            leader.0.lock().unwrap().durability.iter().collect::<Vec<_>>()
        );
        println!(
        ...
        println!(
            ...
            s1.lock().unwrap().durability.omni_paxos.get_current_leader().unwrap()
        );
        println!(
            "New leader, AFTER TIMEOUT, for old leader: {}",
            s3.lock().unwrap().durability.omni_paxos.get_current_leader().unwrap()
        );
    ...
    #[test]
    // Test Case #4: Simulate the 3 partial connectivity scenarios from the OmniPaxos liveness lecture.
    // Does the system recover? *NOTE* for this test you may need to modify the messaging logic.
                ...
                for pid1 in SERVERS {
                    if pid1 != leader_to_be {
                        server_to_get
                            .lock()
                            .unwrap()
                            .connected_nodes[pid1 as usize - 1] = false;
                    }
                }
            }
        }
    ...
    fn test_case_4_constrained() {
        ...
        // Step 1
        // For all the servers that are not the leader and it should only be connected to a common server that is not yet a leader
        for pid in SERVERS {
            if pid != leader {
                let (server, _) = nodes.get(&pid).unwrap();
    ...
    fn test_case_4_chained() {
        ...
        println!("Newly elected leader {:?}", new_leader);
        println!(
            "Current offset should be the same as server2, 7: {}",
            leader.durability.get_durable_tx_offset().0
        );
        println!(
            "Current offset should be the same as server2, 7: {}",
            leader.datastore.get_cur_offset().unwrap().0
        );
    }
